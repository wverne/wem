...
// private constants
const double EOS::ITERATE_PRECISION = 0.000001;
const double EOS::DIFF_PRECISION    = 0.000001;
...
EOS::EOS(const EOS& copy_from)
{
   ...
   else if (isTabular)
   {
      ...
      this->tabP = new double[tabNumVals];
      this->tabRho = new double[tabNumVals];
      this->tabdPdRho = new double[tabNumVals];
      this->tabd2PdRho2 = new double[tabNumVals];
   ...
   if (isThermal)
   {
      ...
      this->debX = new double[debNumVals];
      this->debY = new double[debNumVals];
...
void EOS::setMixTab(string filename)
{
   ...
   tabP        = new double[tabNumVals];
   tabRho      = new double[tabNumVals];
   tabdPdRho   = new double[tabNumVals];
   tabd2PdRho2 = new double[tabNumVals];
...
void EOS::setThermal(int newThermNum)
{
   ...
   if (!isThermal)
   {
	 ...
	 debNumVals++;
      ...
      debX       = new double[debNumVals];
      debY       = new double[debNumVals];
...
double EOS::getPMix(double rho, double T)
{
   ...
   while (abs((rho - rhoNew)/rho) > 0.000000001)
   {
      double drhodP = (getRhoMix(P * (1 + epsilon), T) 
		           - getRhoMix(P * (1 - epsilon), T)) 
	                   / (2 * P * epsilon);
      P -= (rhoNew - rho) / drhodP;
