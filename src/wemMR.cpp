/* 
 * wemMR: Mass-Radius Plot Generator
 * Wesley Verne
 */
...
int main(int argc, const char* argv[])
{
    ...
    EOS* eosc = new EOS();
    eosc->setNum(1);
    double interval = M_EARTH;
    double startMass = M_EARTH;
    double endMass = 10 * M_EARTH;
    string oFilename = "wemMR_out";
    bool verbose = false;

    PlanetComp planetComp = PlanetComp(1000, eosc);
	...
	else if (!strcmp("-m", argv[i]) || !strcmp("-mass", argv[i]))
	{
	    ...
	    startMass = atof(argv[i+1]) * M_EARTH;
	    endMass = atof(argv[i+2]) * M_EARTH;
	    i += 3;
	}
    ...
    planetComp.printMR(startMass, endMass, interval, oFilename);
...
void printHelp(const char* progName)
{
    ...
    printf("           -interval/-i output_interval (Earth masses)\n");
    printf("           -mass/-m start_mass end_mass (Earth masses)\n");
